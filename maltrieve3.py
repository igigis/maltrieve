...
class config(object):
    ...
    def __init__(self, args, filename='maltrieve.cfg'):
        ...
        if self.configp.has_option('Maltrieve', 'black_list'):
            self.black_list = self.configp.get(
                'Maltrieve', 'black_list').strip().split(',')
        else:
            self.black_list = []

        if self.configp.has_option('Maltrieve', 'white_list'):
            self.white_list = self.configp.get(
                'Maltrieve', 'white_list').strip().split(',')
        else:
        ...
        if not os.path.exists(self.dumpdir):
            try:
                ...
                logging.error(
                    'Could not create %s, using default', self.dumpdir)
                self.dumpdir = '/tmp/malware'
        ...
        try:
            ...
            logging.error(
                'Could not open %s for writing, using default', self.dumpdir)
            self.dumpdir = '/tmp/malware'
        else:
            os.close(fd)
            os.remove(temp_path)
        ...
        # TODO: Merge these
        self.vxcage = args.vxcage or self.configp.has_option(
            'Maltrieve', 'vxcage')
        self.cuckoo = args.cuckoo or self.configp.has_option(
            'Maltrieve', 'cuckoo')
        ...
        self.viper = args.viper or self.configp.has_option(
            'Maltrieve', 'viper')
...
def upload_crits(response, md5, cfg):
    if response:
        ...
        zip_files = ['application/zip', 'application/gzip',
        ...
        try:
            ...
            domain_response = requests.post(
                url, headers=headers, data=domain_data, verify=False)
            ...
            if domain_response.status_code == requests.codes.ok:
                ...
                logging.info("Submission of %s failed: %d",
                             url, domain_response.status_code)
        except requests.exceptions.ConnectionError:
            logging.info(
                "Could not connect to CRITs when submitting domain %s", domain_data['domain'])
        except requests.exceptions.HTTPError:
            logging.info(
                "HTTP error when submitting domain %s to CRITs", domain_data['domain'])
        ...
        try:
            ...
            sample_response = requests.post(
                url, headers=headers, files=files, data=sample_data, verify=False)
            # pylint says "Instance of LookupDict has no 'ok' member" but it's wrong, I checked
            if sample_response.status_code == requests.codes.ok:
                ...
                if sample_response_data['return_code'] == 0:
                    ...
                    logging.info(
                        "Submitted sample %s to CRITs, response was %r", md5, sample_response_data)
            else:
                logging.info("Submission of sample %s failed: %d}",
                             md5, sample_response.status_code)
        except requests.exceptions.ConnectionError:
            logging.info(
                "Could not connect to CRITs when submitting sample %s", md5)
        except requests.exceptions.HTTPError:
        ...
        if (inserted_sample and inserted_domain):
            ...
            try:
                ...
                relationship_response = requests.post(
                    url, headers=headers, data=relationship_data, verify=False)
                ...
                logging.info(
                    "Could not connect to CRITs when submitting relationship for sample %s", md5)
            except requests.exceptions.HTTPError:
                logging.info(
                    "HTTP error when submitting relationship for sample %s to CRITs", md5)
                return True
        else:
            return False
    ...
    if response:
        ...
        try:
            ...
            response = requests.post(
                url, headers=headers, files=files, data=tags)
            response_data = response.json()
            logging.info("Submitted %s to VxCage, response was %d",
                         md5, response_data["message"])
        except requests.exceptions.ConnectionError:
            logging.info(
                "Could not connect to VxCage, will attempt local storage")
            return False
        else:
            return True
...
def upload_cuckoo(response, md5, cfg):
    if response:
        ...
        try:
            ...
            response_data = response.json()
            logging.info("Submitted %s to Cuckoo, task ID %d",
                         md5, response_data["task_id"])
        except requests.exceptions.ConnectionError:
            logging.info(
                "Could not connect to Cuckoo, will attempt local storage")
            return False
        else:
            return True
    ...
    if response:
        ...
        try:
            ...
            response = requests.post(
                url, headers=headers, files=files, data=tags)
            response_data = response.json()
            logging.info("Submitted %s to Viper, response was %s",
                         md5, response_data["message"])
        except requests.exceptions.ConnectionError:
            logging.info(
                "Could not connect to Viper, will attempt local storage")
            return False
        else:
            return True
...
def process_malshare(response, cfg):
    ...
    return set([
        "https://api.malshare.com/sampleshare.php?action=getfile&api_key=%s&hash=%s" % (
            api_key, file_hash)
...
def setup_args(args):
    ...
    parser.add_argument(
        "--config", help="Alternate config file (default maltrieve.cfg)")
...
def main():
    ...
    if cfg.malshare_key:
        source_urls['https://www.malshare.com/daily/malshare.current.txt'] = lambda x: process_malshare(
            x, cfg)
    headers = {'User-Agent': 'Maltrieve'}
